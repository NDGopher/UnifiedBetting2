...
// 5. Fixed selectors to match actual POD page structure
...
(function() {
            ...
            setTimeout(() => {
                ...
                const intervalId = setInterval(async () => {
                    ...
                    try {
                        ...
                        console.warn(`[waitForSniffedEventId] Attempt ${attempts}: Error:`, error);
    ...
    async function processAlert(row) {
        ...
        if (sniffedEventId) {
            ...
            if (alertData.eventIdFromRowAttribute) {
                ...
                processedRows.set(rowId, { hash: alertHash, timestamp: now, eventId: null }); // Mark as processed to avoid loops
                return;
            }
        }
        ...
        try {
            ...
            if (response && response.status === "success") {
                console.log(`[processAlert] Successfully forwarded to Python for ${rowId}:`, response);
            } else {
                console.error(`[processAlert] Failed to forward to Python for ${rowId}:`, response);
            }
        } catch (error) {
            console.error(`[processAlert] Error forwarding to Python for ${rowId}:`, error);
        }

        processedRows.set(rowId, { hash: alertHash, timestamp: now, eventId: alertData.eventId });
    }
    ...
    // Check if Firefox extension APIs are available
    if (typeof browser === 'undefined' || !browser.runtime) {
        console.error("🚨🚨🚨 Firefox extension APIs not available! Extension may not be properly loaded. 🚨🚨🚨");
        // Wait a bit and try again
        setTimeout(() => {
            if (typeof browser !== 'undefined' && browser.runtime) {
                console.log("🚨🚨🚨 Firefox APIs now available - starting monitoring 🚨🚨🚨");
                startMonitoring();
            } else {
                console.error("🚨🚨🚨 Firefox APIs still not available after retry 🚨🚨🚨");
            }
        }, 2000);
    } else {
        console.log("🚨🚨🚨 Firefox extension APIs available - starting monitoring 🚨🚨🚨");
        startMonitoring();
    }
