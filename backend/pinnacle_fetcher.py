...
def fetch_live_pinnacle_event_odds(event_id):
    ...
    for attempt in range(max_retries + 1):
        try:
            if attempt > 0:
                print(f"[Pinnacle Fetcher] Retry attempt {attempt} for {event_id}")
            ...
            if attempt < max_retries:
                print(f"[Pinnacle Fetcher] Timeout on attempt {attempt + 1}, retrying...")
                continue
            else:
                error_message = f"Request timed out after {max_retries + 1} attempts"
